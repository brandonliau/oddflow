...
// run can resume without re-recording or double-counting tickers it already
// has. Finalize rewrites the file sorted and unique.
type FileSet struct {
	mu   sync.Mutex
	f    *os.File
...
// Has reports whether t has been recorded.
func (s *FileSet) Has(t string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	_, ok := s.seen[t]
	return ok
...
// Close closes the underlying file without rewriting it (used for checkpoints).
func (s *FileSet) Close() error { return s.f.Close() }

// listingState persists progress of a single cursor-paginated listing so it can
